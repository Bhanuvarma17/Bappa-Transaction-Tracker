...
function loadDatabase() {
  ...
  if (fs.existsSync(STORE_FILE)) {
    try {
      ...
      db = JSON.parse(data);
      console.log(`Database loaded: ${db.users.length} users, ${db.profiles.length} profiles, ${db.expenses.length} expenses.`);
      return;
    } catch (err) {
      console.error("Failed to parse store.json, initializing empty db:", err);
    }
  }

  // Seed sample initial committee for "SBVMB Youth" as requested in prompt!
  seedDefaultData();
  saveDatabase();
}
